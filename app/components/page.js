...
define(function(require,exports,module){
    ...
    pagination.directive('uiPagination', ['$template',function($template) {
        return {
            restrict : 'EA',
            templateUrl : $template('template/pagination'),
            replace : true,
            scope : {
                conf : '=?'
            },
            link : function(scope , ele , attrs){
                ...
                scope.$watch('conf',function(newValue){
                    // 初始化一页展示多少条  默认为10
                    ...
                    scope.pageListFn = function(){
                        ...
                        if(page.limit <  page.defaultLimit ){
                            ...
                            if(conf.currentPage < 4){
                                ...
                                for(var i= conf.currentPage -2 ; i< conf.currentPage*1 + 2 ; i++){
                                    scope.pageList.push(i);
                                }
                                scope.pageList.push('...' , page.limit );
                                scope.pageList.unshift(1 , '...');
